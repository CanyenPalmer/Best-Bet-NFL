...
def evaluate_single(req: SingleBetReq) -> SingleBetResp:
    ...
    if market == "prop":
        res = engine.compute_prop_probability(
            ...
            kind=req.get("prop_kind", ""),
            side=req.get("side", "over"),
            line=float(req.get("line", 0.0))
        )
        p = float(res["p_hit"]) if res else 0.0
        label = f"{req.get('player','')} {req.get('side','').upper()} {req.get('line','')} {req.get('prop_kind','')} vs {req.get('opponent','')}"
        ...
        snapshot, debug = ml.get("snapshot", {}), {"expected_points_for": ml["expected_points_for"], "expected_points_against": ml["expected_points_against"]}
        ...
        p = 1.0 - engine._norm_cdf(line, mu_diff, SD_DIFF) if side == "home" else engine._norm_cdf(line, mu_diff, SD_DIFF)
        ...
        p_over = 1.0 - engine._norm_cdf(line, mu_total, SD_TOTAL)
        ...
        p_over = 1.0 - engine._norm_cdf(line, mu_team, SD_TEAM)
        p = p_over if side == "over" else 1.0 - p_over
...
def evaluate_parlay(req: ParlayReq) -> ParlayResp:
    ...
    return {
        ...
        "correlation_note": "Assumes independent legs. Stats from same team may be correlated.",
